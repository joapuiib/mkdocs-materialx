...
!!! warning "Legacy Syntax Warning"
    The previous method for customizing admonition icons is deprecated.
    Please migrate to the new configuration format below to ensure your
    custom icons continue to work following this version.

Each of the supported admonition types has a distinct icon and color profile.
These can be changed to any icon bundled with the theme, or even a [custom icon].
Furthermore, you can define entirely new, custom admonition types
by mapping them to specific icons and colors.
...
Also, style overrides can be applied to the default admonition types or for custom types.
All you need is a color or an `*.svg` icon. Copy the icon's code from the [`.icons`][custom icons] folder
